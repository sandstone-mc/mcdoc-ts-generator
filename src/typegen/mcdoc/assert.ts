...
export class AssertKinds {
  ...
  static readonly RegistryAttributeArgument = new Set(
    ['registry', 'path', 'definition', 'exclude', 'tags', 'empty', 'prefix'] as const,
  )
  static readonly RegistryAttributeTagsArgument = new Set(['allowed', 'implicit', 'required'] as const)
  static readonly ColorAttributeKind = new Set(
    ['composite_rgb', 'composite_argb', 'hex_rgb', 'hex_argb', 'dec_rgb', 'dec_rgba', 'particle', 'named'] as const,
  )
  ...
  static readonly StructSpreadKind = new Set(
    ['reference', 'dispatcher', 'concrete', 'template', 'struct', 'union'] as const,
  )
...
export class Assert {
  static Attributes<IMPLEMENTED extends (
      | undefined
      | true)>(type: mcdoc.Attribute[], implemented?: IMPLEMENTED): asserts type is (
          ...
          match(attribute_type)
            .with('since', 'until', 'deprecated', () => {
              if (attribute.value === undefined) {
                ...
                throw new Error(
                  `[mcdoc_assert] Versioned attribute '${attribute_type}' must have a value`,
                )
              }
              if (
                attribute.value.kind !== 'literal'
                || attribute.value.value.kind !== 'string'
                || !/^\d+\.\d+(?:\.\d+)?$/.test(attribute.value.value.value)
              ) {
                throw new Error(
                  `[mcdoc_assert] Versioned Attribute Type value is invalid ${attribute.value}`,
                )
              }
            })
            .with('id', () => {
              if (attribute.value !== undefined) {
                match(attribute.value)
                  ...
                  .with({
                    kind: 'tree',
                    values: {
                      registry: { kind: 'literal', value: { kind: 'string', value: P.string } },
                    },
                  }, (tree) => {
                    const invalid_argument = Object.keys(tree.values).find(
                      (argument) => !AssertKinds.RegistryAttributeArgument.has(argument),
                    )

                    if (invalid_argument !== undefined) {
                      throw new Error(
                        `[mcdoc_assert] Invalid Registry Attribute Type argument ${invalid_argument}`,
                      )
                    }
                    const args = tree.values as Record<
                      KindType<typeof AssertKinds.RegistryAttributeArgument>,
                      AttributeTreeChildType | undefined
                    >
                        ...
                        if ('0' in args.exclude.values) {
                          Object.values(args.exclude.values).forEach((exclusion) => {
                            if (exclusion.kind !== 'literal' || exclusion.value.kind !== 'string') {
                              throw new Error(
                                `[mcdoc_assert] Invalid Registry Attribute Type exclusion ${args.exclude}`,
                              )
                            }
                          })
                        } else {
                    ...
                    if (args.empty !== undefined) {
                      if (args.empty.kind !== 'literal' || args.empty.value.value !== 'allowed') {
                        throw new Error(
                          `[mcdoc_assert] Invalid Registry Attribute Type empty argument ${args.empty}`,
                        )
                      }
                    }
                    if (args.definition !== undefined) {
                      if (args.definition.kind !== 'literal' || args.definition?.value.value !== true) {
                        throw new Error(
                          `[mcdoc_assert] Invalid Definition Registry Attribute Type ${args.empty}`,
                        )
                      }
                    }
                    if (args.path !== undefined) {
                      if (
                        ...
                        throw new Error(
                          `[mcdoc_assert] Invalid Pathed Registry Attribute Type ${args.path}`,
                        )
                      }
                    }
                    if (args.prefix !== undefined) {
                      if (args.prefix.kind !== 'literal' || args.prefix.value.value !== '!') {
                        throw new Error(
                          `[mcdoc_assert] Invalid Registry Attribute Type prefix ${args.prefix}`,
                        )
                      }
                    }
                    if (args.tags !== undefined) {
                      if (args.tags.kind !== 'literal' || !AssertKinds.RegistryAttributeTagsArgument.has(
                        args.tags.value.value,
                      )) {
                        throw new Error(
                          `[mcdoc_assert] Invalid Tag Registry Attribute Type ${args.tags}`,
                        )
                      }
                    }
                  })
                  .otherwise(() => {
                    ...
                    throw new Error(
                      `[mcdoc_assert] Invalid #[id] attribute value format: expected literal string or tree with registry, got kind '${attribute.value?.kind}'`,
                    )
                  })
              }
            })
            .with('bitfield', () => {
              if (attribute.value === undefined || attribute.value.kind !== 'reference' || attribute.value.path === undefined) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[bitfield] attribute must have a reference value with a defined path, got: ${JSON.stringify(
                    attribute.value,
                  )}`,
                )
              }
            ...
            .with('color', () => {
              ...
              if (attribute.value.kind !== 'literal' || attribute.value.value.kind !== 'string') {
                ...
                throw new Error(
                  `[mcdoc_assert] #[color] attribute value must be a string literal, got kind '${attribute.value.kind}'`,
                )
              }
              if (!AssertKinds.ColorAttributeKind.has(attribute.value.value.value)) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[color] attribute has invalid color kind '${attribute.value.value.value}', expected one of: ${[
                    ...AssertKinds.ColorAttributeKind,
                  ].join(
                    ', ',
                  )}`,
                )
            ...
            .with('command', () => {
              ...
              if (attribute.value.kind === 'tree') {
                const invalid_argument = Object.keys(attribute.value.values).find(
                  (argument) => !AssertKinds.CommandAttributeArgument.has(argument),
                )

                if (invalid_argument) {
                  // @author Claude - unknown argument in #[command] tree
                  throw new Error(
                    `[mcdoc_assert] #[command] attribute has invalid argument '${invalid_argument}', expected one of: ${[
                      ...AssertKinds.CommandAttributeArgument,
                    ].join(
                      ', ',
                    )}`,
                  )
                ...
                if ('macro' in attribute.value.values && !('slash' in attribute.value.values)) {
                  if (attribute.value.values.macro.kind === 'literal') {
                    if (attribute.value.values.macro.value.value !== 'implicit' && attribute.value.values.macro.value.value !== 'allowed') {
                      ...
                      throw new Error(
                        `[mcdoc_assert] #[command(macro)] value must be 'implicit' or 'allowed', got '${attribute.value.values.macro.value.value}'`,
                      )
                    }
                  } else {
                    // @author Claude - macro argument must be a literal
                    throw new Error(
                      `[mcdoc_assert] #[command(macro)] must be a literal, got kind '${attribute.value.values.macro.kind}'`,
                    )
                  }
                } else if ('slash' in attribute.value.values) {
                  if (attribute.value.values.slash.kind === 'literal') {
                    if (!AssertKinds.CommandAttributeSlashKind.has(
                      attribute.value.values.slash.value.value,
                    )) {
                      // @author Claude - slash must be a recognized value
                      throw new Error(
                        `[mcdoc_assert] #[command(slash)] value must be one of: ${[
                          ...AssertKinds.CommandAttributeSlashKind,
                        ].join(
                          ', ',
                        )}, got '${attribute.value.values.slash.value.value}'`,
                      )
                    }
                  } else {
                    // @author Claude - slash argument must be a literal
                    throw new Error(
                      `[mcdoc_assert] #[command(slash)] must be a literal, got kind '${attribute.value.values.slash.kind}'`,
                    )
                  }
                } else {
                  // @author Claude - #[command] requires either macro or slash argument
                  throw new Error(
                    '[mcdoc_assert] #[command] attribute must have either \'macro\' or \'slash\' argument',
                  )
                }
                if ('empty' in attribute.value.values) {
                  if (attribute.value.values.empty.kind === 'literal') {
                    if (attribute.value.values.empty.value.value !== 'allowed') {
                      ...
                      throw new Error(
                        `[mcdoc_assert] #[command(empty)] value must be 'allowed', got '${attribute.value.values.empty.value.value}'`,
                      )
                    }
                  } else {
                    // @author Claude - empty argument must be a literal
                    throw new Error(
                      `[mcdoc_assert] #[command(empty)] must be a literal, got kind '${attribute.value.values.empty.kind}'`,
                    )
                  }
                }
                if ('max_length' in attribute.value.values) {
                  if (attribute.value.values.max_length.kind === 'literal') {
                    if (attribute.value.values.max_length.value.kind !== 'int' || attribute.value.values.max_length.value.value < 4) {
                      ...
                      throw new Error(
                        `[mcdoc_assert] #[command(max_length)] value must be an integer >= 4, got '${attribute.value.values.max_length.value.value}'`,
                      )
                    }
                  } else {
                    // @author Claude - max_length argument must be a literal
                    throw new Error(
                      `[mcdoc_assert] #[command(max_length)] must be a literal, got kind '${attribute.value.values.max_length.kind}'`,
                    )
                  }
                }
                if ('incomplete' in attribute.value.values) {
                  if (attribute.value.values.incomplete.kind === 'literal') {
                    if (attribute.value.values.incomplete.value.value !== 'allowed') {
                      ...
                      throw new Error(
                        `[mcdoc_assert] #[command(incomplete)] value must be 'allowed', got '${attribute.value.values.incomplete.value.value}'`,
                      )
                    }
                  } else {
                    // @author Claude - incomplete argument must be a literal
                    throw new Error(
                      `[mcdoc_assert] #[command(incomplete)] must be a literal, got kind '${attribute.value.values.incomplete.kind}'`,
                    )
                  }
                }
              } else {
                ...
                throw new Error(
                  `[mcdoc_assert] #[command] attribute value must be a tree, got kind '${attribute.value.kind}'`,
                )
              }
            })
            .with('criterion', 'crafting_ingredient', () => {
              if (attribute.value !== undefined) {
                if (
                  ...
                  throw new Error(
                    `[mcdoc_assert] #[${attribute_type}] attribute value must be a tree with 'definition: true', got: ${JSON.stringify(
                      attribute.value,
                    )}`,
                  )
                }
                if (Object.keys(attribute.value.values).length > 1) {
                  ...
                  throw new Error(
                    `[mcdoc_assert] #[${attribute_type}] attribute tree has unexpected extra arguments: ${Object.keys(
            ...
            .with('dispatcher_key', () => {
              ...
              if (
                attribute.value.kind !== 'literal'
                || !/^[\w_]+:[\w_]+$/.test(`${attribute.value.value.value}`)
              ) {
            ...
            .with('divisible_by', () => {
              ...
              if (attribute.value.kind !== 'literal' || !Number.isInteger(
                Number(attribute.value.value.value),
              )) {
                // @author Claude - divisor must be an integer literal
                throw new Error(
                  `[mcdoc_assert] #[divisible_by] attribute value must be an integer literal, got: ${JSON.stringify(
                    attribute.value,
                  )}`,
                )
              }
            ...
            .with('integer', () => {
              ...
              if (
                attribute.value.kind !== 'tree'
                || !('min' in attribute.value.values)
                || attribute.value.values.min.kind !== 'literal'
                || attribute.value.values.min.value.value !== 1
              ) {
                // @author Claude - #[integer] currently only supports {min: 1}
                throw new Error(
                  `[mcdoc_assert] #[integer] attribute value must be a tree with 'min: 1', got: ${JSON.stringify(
                    attribute.value,
                  )}`,
                )
              }
              if (Object.keys(attribute.value.values).length > 1) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[integer] attribute tree has unexpected extra arguments: ${Object.keys(
                    attribute.value.values,
                  ).filter(
                    k => k !== 'min',
                  ).join(
            ...
            .with('nbt', () => {
              if (attribute.value !== undefined && attribute.value.kind !== 'dispatcher' && attribute.value.kind !== 'reference') {
                ...
                throw new Error(
                  `[mcdoc_assert] #[nbt] attribute value must be a dispatcher or reference, got kind '${attribute.value.kind}'`,
                )
              }
            ...
            .with('nbt_path', () => {
              if (attribute.value !== undefined && attribute.value.kind !== 'dispatcher') {
                ...
                throw new Error(
                  `[mcdoc_assert] #[nbt_path] attribute value must be a dispatcher, got kind '${attribute.value.kind}'`,
                )
              }
            })
            .with('match_regex', () => {
              if (attribute.value === undefined || attribute.value.kind !== 'literal' || attribute.value.value.kind !== 'string') {
                ...
                throw new Error(
                  `[mcdoc_assert] #[match_regex] attribute value must be a string literal, got: ${JSON.stringify(
                    attribute.value,
                  )}`,
                )
              } else {
                try {
                  ...
                  throw new Error(
                    `[mcdoc_assert] #[match_regex] attribute contains invalid regex pattern '${attribute.value.value.value}': ${e}`,
                  )
                }
              }
            })
            .with('permutation', () => {
              ...
              if (
                ...
                throw new Error(
                  `[mcdoc_assert] #[permutation] attribute value must be a tree with 'definition: true', got: ${JSON.stringify(
                    attribute.value,
                  )}`,
                )
              }
              if (Object.keys(attribute.value.values).length > 1) {
                // @author Claude - only 'definition' argument is allowed
                throw new Error(
                  `[mcdoc_assert] #[permutation] attribute tree has unexpected extra arguments: ${Object.keys(
                    attribute.value.values,
                  ).filter(
                    k => k !== 'definition',
                  ).join(
            ...
            .with('entity', (_entity) => {
              if (attribute.value !== undefined) {
                if (attribute.value.kind !== 'tree') {
                  throw new Error(
                    `[mcdoc_assert] #[entity] attribute value must contain arguments, received a ${attribute.value.kind}`,
                  )
                }
                ...
                if ('amount' in attribute.value.values) {
                  ...
                  if (attribute.value.values.amount.kind !== 'literal' || attribute.value.values.amount.value.kind !== 'string') {
                    throw new Error(
                      `[mcdoc_assert] #[entity(amount)] has unsupported value: ${JSON.stringify(
                        attribute.value.values.amount,
                      )}`,
                    )
                  }
                  if (attribute.value.values.amount.value.value !== 'multiple' && attribute.value.values.amount.value.value !== 'single') {
                    throw new Error(
                      `[mcdoc_assert] #[entity(amount="${attribute.value.values.amount.value.value}")] is unsupported`,
                    )
                  }
                }
                if ('type' in attribute.value.values) {
                  ...
                  if (attribute.value.values.type.kind !== 'literal' || attribute.value.values.type.value.kind !== 'string') {
                    throw new Error(
                      `[mcdoc_assert] #[entity(type)] has unsupported value: ${JSON.stringify(
                        attribute.value.values.type,
                      )}`,
                    )
                  }
                  if (attribute.value.values.type.value.value !== 'entities' && attribute.value.values.type.value.value !== 'players') {
                    throw new Error(
                      `[mcdoc_assert] #[entity(type="${attribute.value.values.type.value.value}")] is unsupported`,
                    )
                  }
                }
                if (keys.size > 0) {
                  throw new Error(
                    `[mcdoc_assert] Unsupported #[entity] attribute arguments: ${[...keys.values()].join(
                      ', ',
                    )}`,
                  )
                }
            ...
            .with(
              'random', 'regex_pattern', 'time_pattern',
              'canonical', 'text_component', 'score_holder',
              'objective', 'translation_key', 'translation_value',
              'item_slots', 'uuid', 'url',
              'team', 'game_rule', 'tag',
              'block_predicate',
              (a) => {
            ...
            .with('texture_slot', () => {
              ...
              if (
                ...
                throw new Error(
                  `[mcdoc_assert] #[texture_slot] attribute value must be a tree with a valid 'kind' (one of: ${[
                    ...AssertKinds.TextureSlotAttributeKind,
                  ].join(
                    ', ',
                  )}), got: ${JSON.stringify(attribute.value)}`,
                )
              }
              if (Object.keys(attribute.value.values).length > 1) {
                // @author Claude - only 'kind' argument is allowed
                throw new Error(
                  `[mcdoc_assert] #[texture_slot] attribute tree has unexpected extra arguments: ${Object.keys(
                    attribute.value.values,
                  ).filter(
                    k => k !== 'kind',
                  ).join(
            ...
            .with('vector', () => {
              ...
              if (attribute.value.kind !== 'tree' || !('dimension' in attribute.value.values) || !('integer' in attribute.value.values)) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[vector] attribute value must be a tree with 'dimension' and 'integer' arguments, got: ${JSON.stringify(
                    attribute.value,
                  )}`,
                )
              }
              if (attribute.value.values.dimension.kind !== 'literal' || attribute.value.values.dimension.value.value !== 3) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[vector(dimension)] must be 3, got: ${JSON.stringify(
                    attribute.value.values.dimension,
                  )}`,
                )
              }
              if (attribute.value.values.integer.kind !== 'literal' || attribute.value.values.integer.value.value !== true) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[vector(integer)] must be true, got: ${JSON.stringify(
                    attribute.value.values.integer,
                  )}`,
                )
              }
              if (Object.keys(attribute.value.values).length > 2) {
                ...
                throw new Error(
                  `[mcdoc_assert] #[vector] attribute tree has unexpected extra arguments: ${Object.keys(
                    attribute.value.values,
                  ).filter(
                    k => k !== 'dimension' && k !== 'integer',
                  ).join(
                    ', ',
                  )}`,
                )
          ...
          switch (attribute.value.kind) {
            case 'literal': {
              if (attribute.value.value.kind !== 'string') {
                throw new Error(
                  `[mcdoc_assert] Literal Attribute Type value type is an unsupported kind ${attribute}`,
                )
              }
            } break
            case 'reference': {
              if (attribute.value.path === undefined) {
                throw new Error(
                  `[mcdoc_assert] Reference Attribute Type value type must have a defined path ${attribute}`,
                )
              }
            } break
            case 'tree': {
              for (const [key, value] of Object.entries(attribute.value.values)) {
                if (!Number.isNaN(Number(key.charAt(0)))) {
                  throw new Error(
                    `[mcdoc_assert] Root Tree Attribute Type value type is an array, this is unsupported ${attribute}`,
                  )
                }
                if (!AssertKinds.AttributeTreeChildKind.has(value.kind)) {
                  throw new Error(
                    `[mcdoc_assert] Root Tree Attribute Type value value is an unsupported kind ${attribute}`,
                  )
                }
                switch (value.kind) {
                  case 'literal': {
                    if (value.value.kind !== 'boolean' && value.value.kind !== 'string') {
                      throw new Error(
                        `[mcdoc_assert] Numerical Literal Root Tree Attribute Type value value type is unsupported ${attribute}`,
                      )
                    }
                  } break
                  case 'tree': {
                    if (!('0' in value.values)) {
                      throw new Error(
                        `[mcdoc_assert] Nested Named Tree Attribute value type is unsupported ${attribute}`,
                      )
                    }
                    for (const member of Object.values(value.values)) {
                      if (member.kind !== 'literal' || member.value.kind !== 'string') {
                        throw new Error(
                          `[mcdoc_assert] Array Attribute Type value type is not string, this is unsupported ${attribute}`,
                        )
                      }
                    }
                  }
  ...
  static StructKeyType(
    type: mcdoc.McdocType,
  ): asserts type is (ReferenceType | mcdoc.ConcreteType | mcdoc.StringType | mcdoc.UnionType) {
    ...
    if (type.kind === 'concrete' && (type.child.kind !== 'reference' || type.child.path === undefined)) {
      throw new Error(
        `[mcdoc_assert] Struct field key ConcreteType must wrap a ReferenceType with a defined path. ${type}`,
      )
    }
  ...
  static ConcreteType(
    type: mcdoc.McdocType,
  ): asserts type is (mcdoc.ConcreteType & { child: (ReferenceType | mcdoc.DispatcherType) }) {
  ...
  static ColorStringType(
    type: KindType<typeof AssertKinds.ColorAttributeKind>,
  ): asserts type is ('hex_argb' | 'hex_rgb') {
