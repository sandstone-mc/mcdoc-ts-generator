  ...
  return (args: Record<string, unknown>) => {
    ...
    for (const item of tuple.items) {
      ...
      if ('child_dispatcher' in value) {
        ...
        child_dispatcher!.push(...((value.child_dispatcher as NonEmptyList<[
          number,
          string,
        ]>).map(([parent_count, property]) => {
          if (parent_count === 0) {
            throw new Error(
              `[mcdoc_tuple] Tuple contains a dynamic dispatcher with invalid parenting: ${item}`,
            )
          }
