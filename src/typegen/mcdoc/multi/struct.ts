...
type ResolvedSpreadType = ReturnType<ReturnType<(typeof TypeHandlers[(
  | 'reference'
  | 'dispatcher'
  | 'concrete'
  | 'template'
    ...
    for (const field of struct.fields) {
      ...
      match(field)
        .with({ kind: 'pair', key: P.string, ...FieldProperties }, (pair) => {
          const value = TypeHandlers[pair.type.kind](pair.type)({
            ...args,
            name: `${name}${pascal_case(pair.key)}`,
          })
            ...
            .with('string', () => {
              if (pair.key.attributes === undefined) {
                ...
                inherit.push(Bind.MappedType(
                  ((('lengthRange' in pair.key && 'min' in pair.key.lengthRange)
                    ? pair.key.lengthRange.min
                    : 0) >= 1 ?
                    Bind.NonEmptyString
                ...
                match(attribute)
                  .with({ name: 'id' }, (attr) => {
                    ...
                    const symbols = 'symbols' in args ?
                      (args.symbols as SymbolUtil | undefined)
                      : undefined
                  ...
                  .with({ name: 'crafting_ingredient' }, () => {
                    ...
                    imports = add_import(
                      imports,
                      `sandstone::arguments::${CRAFTING_INGREDIENT}`,
                    ) // 'A' | 'B' | 'C' ...
                  ...
                  .otherwise(() => {
                    throw new Error(
                      `[mcdoc_struct] Unsupported dynamic key attribute: ${attribute}`,
                    )
                  })
              }
            })
      ...
      const new_list = child_dispatcher.flatMap(([parent_count, property]) => {
        if (parent_count === 0) {
          ...
          if (generic_prop === undefined) {
            ...
            throw new Error(
              `[mcdoc_struct] Received an invalid dynamic dispatcher trying to access '${property}'`,
            )
          }
          ...
          if ('--mcdoc_has_non_indexable' in indexed_access_type!) {
            ...
            const id_ref = indexed_access_type['--mcdoc_id_ref'] as {
              ref: ts.TypeReferenceNode,
              alt: ts.ParenthesizedTypeNode;
            }
            // @ts-ignore
