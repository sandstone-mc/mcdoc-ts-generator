 ...
 */
function SymbolMapIndex(
  symbol_name: string,
  key: ts.TypeNode,
  generics: ts.TypeNode[],
  fallback_info?: FallbackInfo,
): { type: ts.TypeNode, needs_import: boolean } {
 ...
 * 2. Otherwise → RootNBT (since this is intersection context)
 */
function SymbolMapSubIndex(
  symbol_name: string,
  member_key: ts.TypeNode,
  generics: ts.TypeNode[],
  sub_index: NonEmptyList<string>,
  fallback_info?: FallbackInfo,
): { type: ts.TypeNode, needs_import: boolean } {
  const symbol_type = factory.createTypeReferenceNode(symbol_name, generics.length === 0 ? undefined : generics)
  ...
  return (args: Record<string, unknown>) => {
    ...
    if (indices.length === 1 && indices[0].kind === 'dynamic' && typeof indices[0].accessor.at(-1) === 'string') {
      ...
      if (args.root_type) {
        ...
        const indexed_result = SymbolMapIndex(
          symbol_name,
      ...
      if (indices[0].value === '%fallback') {
        ...
        const static_result = SymbolMapIndex(
          symbol_name,
      ...
      if (args.index_keys !== undefined) {
        ...
        const sub_result = SymbolMapSubIndex(
          symbol_name,
        ...
        const key_result = SymbolMapIndex(
          symbol_name,
          factory.createTypeReferenceNode('Key'),
          generics,
          fallback_info,
