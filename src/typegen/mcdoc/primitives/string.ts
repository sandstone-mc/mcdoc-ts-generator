    ...
    return match(attribute)
      ...
      .with({ name: 'id', value: P.nonNullable }, ({ value }) => {
        ...
        return (args: Record<string, unknown>) => {
          ...
          let Resource:
                      | string | (
                      () => string | undefined)
                      | undefined = () => registry_id in ResourceClasses
                        ? ResourceClasses[registry_id as keyof typeof ResourceClasses]
                        : undefined
          ...
          if (id_attr.kind === 'literal') {
            ...
            if ('path' in id_attr.values) {
              return {
                ...
                docs: [
                  '',
                  `Value: A ${registry_id} ID within a path root of \`(namespace)/textures/${id_attr.values.path!.value.value}\``,
            ...
            if ('exclude' in id_attr.values) {
              exclude = (reg: ts.TypeNode) => factory.createTypeReferenceNode('Exclude', [
                ...
                factory.createParenthesizedType(factory.createUnionTypeNode(
                  Object.values(id_attr.values.exclude!.values).map(
                    (literal) => Bind.StringLiteral(literal.value.value),
                  ),
                )),
              ])
            }
          ...
          const result_type = types.length === 1
            ? types[0]
            : factory.createParenthesizedType(factory.createUnionTypeNode(types))
      ...
      .with({ name: 'item_slots' }, () => {
        ...
        return (args: Record<string, unknown>) => ({
          ...
          imports: {
            ordered: [
              `sandstone::arguments::${ENTITY_SLOTS}`,
              `sandstone::${LiteralUnion}`,
            ] as NonEmptyList<string>,
            check: new Map([
              [`sandstone::arguments::${ENTITY_SLOTS}`, 0],
              [`sandstone::${LiteralUnion}`, 1],
            ]),
          },
        } as const)
      })
